...
# ── Expected feature columns (from the training pipeline) ────────────────────
EXPECTED_FEATURES = [
    "Air temperature [K]",
...
@app.get("/health")
def health():
    return {"status": "ok", "models_loaded": failure_model is not None, "dataset_loaded": session_store.get("df") is not None}
...
async def upload_csv(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Save to disk so it persists across server restarts
...
def get_dashboard_data():
    ...
    try:
        ...
        if "Type" in df.columns:
            ...
            machines = [
                { "name": "Main Unit", "status": "Running" if efficiency > 80 else "Warning", "efficiency": round(efficiency, 1), "color": "#22c55e" }
            ]
            
        alertsData = []
        if "_predicted_failure" in df.columns and "_failure_probability" in df.columns:
            high_risk = df[df["_predicted_failure"] == 1].sort_values("_failure_probability", ascending=False).head(5)
            for idx, row in high_risk.iterrows():
                prob = row["_failure_probability"]
                alertsData.append({
                    "icon": "⚠️", "iconBg": "#ef4444", 
                    "title": f"High Failure Risk ({prob*100:.1f}%)",
                    "desc": f"Anomaly detected at machine index {idx}",
                    "time": "Just now", "badge": "High", "badgeBg": "#ef4444", "read": False
                })
        if len(alertsData) == 0:
            alertsData.append({
                "icon": "✅", "iconBg": "#22c55e", "title": "All Systems Nominal", 
                "desc": "No recent failures predicted", "time": "Just now", 
                "badge": "Info", "badgeBg": "#22c55e", "read": False
            })
...
def get_prediction_data():
    try:
        ...
        # Calculate derived metrics for the prediction page based on actual dataset
        # Assume Air temperature [K], Torque [Nm], Rotational speed [rpm], Tool wear [min]
        ...
        # Heatmap data based on chunks of the dataset (representing weeks)
...
def generate_answer(question: str, df: pd.DataFrame, store: dict) -> str:
    ...
    if any(w in q for w in ["overview", "summary", "describe", "tell me about", "what is this", "what data"]):
        ...
        if predictions:
            msg += f"\n🔴 **Failure Predictions:** {predictions['failure_count']} failures detected out of {len(df)} samples "
            msg += f"({round(predictions['failure_count']/len(df)*100, 1)}% failure rate)\n"
        return msg

    # ── Failure / prediction questions ───────────────────
    if any(w in q for w in ["failure", "fail", "predict", "risk", "danger", "broken", "break"]):
        if predictions is None:
            return ("The uploaded data doesn't contain the required manufacturing features for failure prediction. "
                    "The model expects columns like: Rotational speed, Torque, Tool wear, Temperature, etc.")
        msg = f"🔍 **Failure Prediction Analysis**\n\n"
        ...
        if predictions.get("probabilities"):
            ...
            if high_risk:
                msg += f"⚠️ **{len(high_risk)} samples with >70% failure probability.** "
                msg += f"Highest risk at rows: {', '.join(str(r) for r in high_risk[:10])}\n"
        return msg
    ...
    if any(w in q for w in ["anomal", "outlier", "unusual", "abnormal", "irregular"]):
        ...
        total = len(df)
        msg = f"🔎 **Anomaly Detection Results**\n\n"
    ...
    msg += "• Ask about **failures/predictions** — \"How many failures are predicted?\"\n"
    msg += "• Ask about **anomalies** — \"Are there any anomalies?\"\n"
    msg += "• Ask about **specific columns** — \"Tell me about Torque\"\n"
    msg += "• Ask for **statistics** — \"What's the average temperature?\"\n"
    msg += "• Ask about **correlations** — \"Which features are correlated?\"\n"
    msg += "• Ask about **health status** — \"What's the system health?\"\n"
    msg += "• Ask for an **overview** — \"Give me a summary\"\n"
    return msg
