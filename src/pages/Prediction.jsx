...
const refreshPlantData = prev => ({
  avgTemp:                 Math.min(95,  Math.max(50,  prev.avgTemp  + (Math.random()-0.5)*8)),
  maxVibration:            Math.min(7.5, Math.max(0.5, prev.maxVibration + (Math.random()-0.5)*1.5)),
  avgPressure:             Math.min(80,  Math.max(20,  prev.avgPressure  + (Math.random()-0.5)*6)),
  overallEfficiency:       Math.min(100, Math.max(45,  prev.overallEfficiency + (Math.random()-0.5)*12)),
  maintenanceOverdueCount: Math.max(0,   prev.maintenanceOverdueCount + (Math.random()>0.7?1:Math.random()>0.5?-1:0)),
  errorState:   Math.random() < 0.1,
  warningState: Math.random() < 0.2,
  timestamp: new Date().toISOString(),
});

const predictPlantPerformance = d => {
  let c = 0.85;
  if (d.avgTemp > 80) c -= 0.20; else if (d.avgTemp > 70) c -= 0.05;
  if (d.maxVibration > 5.0) c -= 0.25; else if (d.maxVibration > 3.5) c -= 0.10;
  if (d.avgPressure < 35 || d.avgPressure > 65) c -= 0.15;
  if (d.overallEfficiency < 70) c -= 0.20; else if (d.overallEfficiency < 80) c -= 0.08;
  c -= 0.10 * d.maintenanceOverdueCount;
  if (d.errorState)   c -= 0.40;
  if (d.warningState) c -= 0.10;
  c = Math.min(1, Math.max(0, c));
  return { confidence: c, classification: c < 0.4 ? 'LOW' : c < 0.7 ? 'MEDIUM' : 'HIGH' };
};

const generateExplanation = (d, confidence, classification) => {
  const issues = [];
  if (d.avgTemp > 80) issues.push(`High temperature (${d.avgTemp.toFixed(1)}°C)`);
  else if (d.avgTemp > 70) issues.push('Elevated temperature');
  if (d.maxVibration > 5.0) issues.push(`Critical vibration (${d.maxVibration.toFixed(1)} mm/s)`);
  else if (d.maxVibration > 3.5) issues.push('Elevated vibration');
  if (d.avgPressure < 35 || d.avgPressure > 65) issues.push(`Pressure out of range (${d.avgPressure.toFixed(0)} PSI)`);
  if (d.overallEfficiency < 70) issues.push(`Low efficiency (${d.overallEfficiency.toFixed(0)}%)`);
  else if (d.overallEfficiency < 80) issues.push('Moderate efficiency');
  ...
  if (d.warningState) issues.push('Active warning state');
  ...
  if (confidence < 0.4) msg += 'Immediate intervention required.';
  else if (confidence < 0.7) msg += 'Preventive maintenance recommended.';
...
// ─── Theme tokens (computed once, passed down) ─────────────────────────────
const tokens = (isDark) => ({
...
const CustomTooltip = ({ active, payload, label, t }) => {
  ...
  return (
    <div style={{
      ...
      padding: '10px 14px', boxShadow: '0 8px 24px rgba(0,0,0,0.18)',
...
const Card = ({ children, t, style = {} }) => (
  <div style={{
    ...
    boxShadow: t.surface === '#ffffff' ? '0 2px 16px rgba(0,0,0,0.06)' : '0 4px 24px rgba(0,0,0,0.4)',
    ...style,
  }}>
    {children}
  </div>
    ...
    <div style={{ position:'relative', width:200, height:200 }}>
      <svg width="200" height="200" style={{ transform:'rotate(-90deg)' }}>
        ...
        <circle cx="100" cy="100" r="68" fill="none" stroke={color} strokeWidth="14"
          strokeDasharray={`${confidence*circ} ${circ}`} strokeLinecap="round"
      ...
      <div style={{ position:'absolute', inset:0, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center' }}>
        <span style={{ fontSize:44, fontWeight:800, lineHeight:1, color:t.text, fontFamily:"'DM Mono',monospace" }}>
          {Math.round(confidence*100)}<span style={{ fontSize:20, fontWeight:500 }}>%</span>
        </span>
...
const Dashboard = () => {
  ...
  const [prediction, setPrediction] = useState(() => {
    ...
    const p = predictPlantPerformance(d);
  ...
  // Dynamic chart states driven by uploaded dataset
  const [dynRiskData, setDynRiskData] = useState(riskDistributionData);
  const [dynEfficiency, setDynEfficiency] = useState(efficiencyTrends);
  const [dynSensorTrends, setDynSensorTrends] = useState(sensorTrends);
  ...
  // Recalculate ML prediction whenever plantData changes (from backend or refresh)
  useEffect(() => {
    const p = predictPlantPerformance(plantData);
  ...
  const fetchPredictionData = async () => {
    try {
      ...
      if (data.has_data) {
        ...
        setDatasetName(data.filename || "Custom Dataset");
        // Rebuild full heatmap structure with new data
        const base = generateHeatmapData();
        setHeatmap({ ...base, data: data.heatmapData });

        // Build dynamic Risk Distribution pie from heatmap weekly efficiency values
        const hd = data.heatmapData;
        const lowCount  = hd.filter(w => w.eff < 65).length;
        const medCount  = hd.filter(w => w.eff >= 65 && w.eff < 80).length;
        const highCount = hd.filter(w => w.eff >= 80).length;
        setDynRiskData([
          { name: 'LOW',    value: Math.max(1, lowCount),  color: '#ef4444' },
          { name: 'MEDIUM', value: Math.max(1, medCount),  color: '#f59e0b' },
          { name: 'HIGH',   value: Math.max(1, highCount), color: '#10b981' },
        ]);

        // Build Efficiency vs Target bar chart from heatmap weeks
        ...
        // Build Sensor Trends line chart from heatmap (temp/vib/pressure per week)
        const days = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];
        setDynSensorTrends(hd.slice(0, 7).map((w, i) => ({
          name: days[i] || `D${i+1}`,
          temp:     parseFloat(w.temp.toFixed(1)),
          vib:      parseFloat(w.vib.toFixed(1)),
          pressure: parseFloat(w.press.toFixed(0)),
        })));
      }
    } catch(err) {
  ...
  const riskBg     = prediction.classification==='HIGH' ? 'rgba(16,185,129,0.10)' : prediction.classification==='MEDIUM' ? 'rgba(245,158,11,0.10)' : 'rgba(239,68,68,0.10)';
  ...
  return (
    <div style={{ minHeight:'100vh', background:t.bg, padding:24, fontFamily:"'DM Sans','Inter',sans-serif", transition:'background 0.3s, color 0.3s' }}>
      <style>{`
      ...
      <div style={{ maxWidth:1280, margin:'0 auto', display:'flex', flexDirection:'column', gap:20 }}>
        ...
        <div className="fade1" style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', alignItems:'center', gap:14 }}>
            ...
            <div>
              <h1 style={{ fontSize:22, fontWeight:800, letterSpacing:-0.5, background:'linear-gradient(90deg,#3b82f6,#06b6d4)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            ...
            <button className="spin-hover" onClick={handleRefresh} style={{
              display:'flex', alignItems:'center', gap:6, padding:'8px 14px', borderRadius:10,
              background:t.surface, border:`1px solid ${t.border}`, color:t.textSub,
              cursor:'pointer', fontSize:12, fontWeight:600,
              animation: pulse ? 'pulseRing .6s ease-out' : 'none',
        ...
        <div className="fade2" style={{ display:'grid', gridTemplateColumns:'220px 1fr 1fr', gap:16 }}>
          <Card t={t} style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:12 }}>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 14px', borderRadius:999, background:riskBg, border:`1px solid ${riskColor}44` }}>
              {prediction.classification==='HIGH' ? <Shield size={14} color={riskColor}/> : prediction.classification==='MEDIUM' ? <AlertCircle size={14} color={riskColor}/> : <XCircle size={14} color={riskColor}/>}
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
            ...
            <MetricTile icon={<Wind size={14}/>} label="Vibration" value={plantData.maxVibration.toFixed(1)} unit="mm/s" accent="#f97316" t={t}/>
            <MetricTile icon={<Gauge size={14}/>} label="Pressure" value={plantData.avgPressure.toFixed(0)} unit="PSI" accent="#3b82f6" t={t}/>
            <MetricTile icon={<Zap size={14}/>} label="Efficiency" value={plantData.overallEfficiency.toFixed(0)} unit="%" accent="#10b981" t={t} trend={plantData.overallEfficiency-75}/>
          </div>
            ...
            <div style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
              ...
              <div style={{ flex:1 }}>
                ...
                <div style={{ marginTop:12, display:'flex', flexWrap:'wrap', gap:6 }}>
                  ...
                  {!plantData.errorState && !plantData.warningState && plantData.maintenanceOverdueCount===0 && <span style={{ fontSize:10, padding:'3px 8px', borderRadius:999, background:'rgba(16,185,129,.12)', color:'#10b981', fontWeight:700 }}>● NOMINAL</span>}
                </div>
              </div>
            </div>
          </Card>
        ...
        <div className="fade3" style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:16 }}>
          ...
          <Card t={t}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={dynEfficiency} barGap={4}>
                ...
                <YAxis domain={[50,100]} tick={axisStyle} axisLine={false} tickLine={false}/>
                <Tooltip content={<CustomTooltip t={t}/>}/>
                <Legend wrapperStyle={legendStyle}/>
          ...
          <Card t={t}>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={dynSensorTrends}>
                <CartesianGrid strokeDasharray="3 3" stroke={t.grid} vertical={false}/>
                <XAxis dataKey="name" tick={axisStyle} axisLine={false} tickLine={false}/>
                ...
                <Line yAxisId="right" type="monotone" dataKey="vib"      name="Vib (mm/s)"     stroke="#f59e0b" strokeWidth={2} dot={false} activeDot={{r:5}}/>
        ...
        {/* Area */}
        <Card t={t} style={{ className:'fade4' }}>
          <CardTitle icon={<Gauge size={15}/>} title="Pressure Area Trend" sub="Daily readings" t={t}/>
          <ResponsiveContainer width="100%" height={150}>
            <AreaChart data={sensorTrends}>
              <defs>
              ...
              <CartesianGrid strokeDasharray="3 3" stroke={t.grid} vertical={false}/>
              <XAxis dataKey="name" tick={axisStyle} axisLine={false} tickLine={false}/>
              <YAxis domain={[40,58]} tick={axisStyle} axisLine={false} tickLine={false}/>
              <Tooltip content={<CustomTooltip t={t}/>}/>
                ...
                {heatmap.metrics.map(metric=>(
                  <tr key={metric.key}>
                    ...
                    {heatmap.data.map((wd,idx)=>{
                      const val=wd[metric.key];
                      return (
                        <td key={idx} style={{
                          padding:'10px 16px', textAlign:'center', borderRadius:8,
                          background:getHeatmapColor(val,metric), color:'#fff',
                          fontWeight:700, fontSize:13, fontFamily:"'DM Mono',monospace",
                          textShadow:'0 1px 3px rgba(0,0,0,.4)', cursor:'default',
        ...
        <div className="fade5" style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
          <Card t={t}>
            ...
            <p style={{ fontSize:12, color:t.textMuted, marginTop:10, lineHeight:1.5 }}>
              ...
              <strong style={{ color:t.text }}>+{plantData.maintenanceOverdueCount*10}%</strong>.
            </p>
          </Card>
        ...
        <p style={{ textAlign:'center', fontSize:11, color:t.textMuted, fontFamily:"'DM Mono',monospace", paddingBottom:8 }}>
          ML models recompute on each refresh · Confidence score ∈ [0,1] · Simulated sensor inputs
        </p>
      </div>
    </div>
  );
